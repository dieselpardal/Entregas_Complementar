...
function createGraph() {
  ...
  //initMap();
    for(var f=0; f<clients.length;f++)
            {
                circle(clients[f].id,centerW+clients[f].x,centerH+clients[f].y,'black',clients[f].name,clients[f].qtd/2);
                print(clients[f].name+":"+clients[f].qtd ,centerW+clients[f].x, centerH+clients[f].y-clients[f].qtd/10, 10,'arial',12,'black');
            }
}
